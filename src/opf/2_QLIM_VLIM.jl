# QLIM + VLIM (Versão Otimizada, Encapsulada e com Resumo Completo)

using PWF
using PowerModels
using JuMP
using Ipopt
using CSV
using DataFrames

print("\033c")

# =========================================================================
# 0. LEITURA DE DADOS E TOPOLOGIA
# =========================================================================
println("1. Lendo arquivo PWF...")

PWF.silence() # Desativa mensagens de log do PWF

caminho_arquivo = joinpath(@__DIR__, "data", "01 MAXIMA NOTURNA_DEZ25.PWF")
data = PWF.parse_file(caminho_arquivo)
base_mva = data["baseMVA"]

PowerModels.select_largest_component!(data)
println("-> Ilhas isoladas removidas! Mantendo apenas a rede principal conectada.")

# Identifica todas as barras definidas como referência (bus_type == 3)
ref_buses = [b_dict for (b_id, b_dict) in data["bus"] if b_dict["bus_type"] == 3]

# Se houver mais de 1, mantemos apenas a primeira e convertemos o resto para PV (Tipo 2)
# Esse comportamento precisa ser revisado, pois pode gerar inconsistências dependendo do caso de teste.
if length(ref_buses) > 1
    println("-> Aviso: Múltiplas barras de referência detectadas. Convertendo excedentes para PV (Tipo 2)...")
    for i in 2:length(ref_buses)
        ref_buses[i]["bus_type"] = 2
    end
end
# ----------------------------------------

# Se houver mais de 1, mantemos apenas a primeira e convertemos o resto para PV (Tipo 2)
if length(ref_buses) > 1
    println("-> Aviso: Múltiplas barras de referência detectadas. Convertendo excedentes para PV (Tipo 2)...")
    for i in 2:length(ref_buses)
        ref_buses[i]["bus_type"] = 2
    end
end
# ----------------------------------------

PowerModels.standardize_cost_terms!(data, order=2)
ref = PowerModels.build_ref(data)[:it][:pm][:nw][0]

# =========================================================================
# 1. INICIALIZAÇÃO DO MODELO E SOLVER
# =========================================================================
model = Model(optimizer_with_attributes(Ipopt.Optimizer, 
    "max_iter" => 500,
    "tol" => 1e-4,
    "print_level" => 5
))

# =========================================================================
# 2. VARIÁVEIS DE ESTADO FÍSICO E ELOS DC
# =========================================================================
println("2. Criando variáveis de estado...")

# declaração das variáveis de módulo e ângulo de tensão para cada barra
# com seus respectivos limites de tensão (vmin, vmax) e valores iniciais (vm, va)
# os valores de "start" vem do arquivo PWF
@variable(model, ref[:bus][i]["vmin"] <= vm[i in keys(ref[:bus])] <= ref[:bus][i]["vmax"], start=ref[:bus][i]["vm"])
@variable(model, va[i in keys(ref[:bus])], start=ref[:bus][i]["va"])

# declaração das variáveis de potência ativa e reativa para cada gerador
# com seus respectivos limites de geração (pmin, pmax, qmin, qmax) e valores iniciais (pg, qg)
@variable(model, ref[:gen][i]["pmin"] <= pg[i in keys(ref[:gen])] <= ref[:gen][i]["pmax"], start=ref[:gen][i]["pg"])
@variable(model, ref[:gen][i]["qmin"] <= qg[i in keys(ref[:gen])] <= ref[:gen][i]["qmax"], start=ref[:gen][i]["qg"])

# declaração das variáveis de potência ativa e reativa para cada linha DC
# nesses casos, as variáveis são fixadas nos valores iniciais do PWF, 
# pois não há otimização de fluxo DC neste modelo
p_dc = Dict(); q_dc = Dict()
for (l, dcline) in ref[:dcline]
    f = dcline["f_bus"]; t = dcline["t_bus"]
    
    p_dc[(l, f, t)] = @variable(model, start=dcline["pf"])
    p_dc[(l, t, f)] = @variable(model, start=dcline["pt"])
    q_dc[(l, f, t)] = @variable(model, start=dcline["qf"])
    q_dc[(l, t, f)] = @variable(model, start=dcline["qt"])
    
    fix(p_dc[(l, f, t)], dcline["pf"]; force=true)
    fix(p_dc[(l, t, f)], dcline["pt"]; force=true)
    fix(q_dc[(l, f, t)], dcline["qf"]; force=true)
    fix(q_dc[(l, t, f)], dcline["qt"]; force=true)
end

# =========================================================================
# 3. LÓGICA DO FLUXO DE POTÊNCIA NAS MÁQUINAS
# =========================================================================

# As barras de referência (Slack) não têm limites de geração, enquanto as 
# demais são fixadas nos valores iniciais do PWF
for (i, gen) in ref[:gen]

    # retira os limites de geração para as barras de referência (Slack)
    if gen["gen_bus"] in keys(ref[:ref_buses])
        if has_lower_bound(pg[i]) delete_lower_bound(pg[i]) end
        if has_upper_bound(pg[i]) delete_upper_bound(pg[i]) end
    else
        # ajusta as barras não-referência para os valores iniciais do PWF
        fix(pg[i], gen["pg"]; force=true)
    end
end

# ajusta as barras de referência (Slack) para o ângulo de tensão zero
for (i, bus) in ref[:ref_buses]
    fix(va[i], 0.0; force=true)
end

# =========================================================================
# 4. VARIÁVEIS E RESTRIÇÕES DE CONTROLE (QLIM e VLIM)
# =========================================================================
PENALIDADE = 1e0 

# declaração das variáveis de desvio de tensão (folga) para cada barra com gerador
gen_buses = unique([gen["gen_bus"] for (i,gen) in ref[:gen]])
@variable(model, sl_v[i in gen_buses], start=0.0)

# declaração das restrições de controle de tensão para cada barra com gerador
# através das variáveis de desvio (sl_v) e dos limites de tensão (vmin, vmax)
# já ajustados para cada uma das barras do sistema
for bus_id in gen_buses
    vm_setpoint = ref[:bus][bus_id]["vm"] 
    @constraint(model, vm[bus_id] == vm_setpoint + sl_v[bus_id])
end

# declaração das variáveis de desvio de carga reativa (folga) para cada 
# barra com carga. Essa variável permite que a carga seja ajustada para 
# atender às restrições de fluxo de potência e será utilizada mais abaixo 
# na restrição de balanço nodal (Leis de Kirchhoff) sendo somada à carga 
# reativa (qd) da barra, permitindo que a carga seja ajustada para atender 
# às restrições de fluxo de potência.
@variable(model, sl_d[i in keys(ref[:load])], start=0.0)

# =========================================================================
# 5. EQUAÇÕES DE FLUXO NOS RAMOS AC
# =========================================================================
println("3. Montando equações de fluxo de potência (AC Polar)...")

# variáveis de fluxo de potência ativa e reativa para cada ramo do sistema
p = Dict(); q = Dict()

# laço para cada ramo do sistema, calculando o fluxo de potência ativa e 
# reativa no sentido de cada barra (De -> Para e Para -> De) através das 
#equações AC Polar.
for (l, branch) in ref[:branch]
    f = branch["f_bus"]; t = branch["t_bus"] # barras de origem e destino do ramo
    g, b = PowerModels.calc_branch_y(branch) # condutância e susceptância do ramo
    tr, ti = PowerModels.calc_branch_t(branch) # transformador (tap) do ramo
    tm = branch["tap"] # fator de transformação do ramo

    # condutância e susceptância do shunt do lado de origem (from)
    g_fr = branch["g_fr"]; b_fr = branch["b_fr"]

    # condutância e susceptância do shunt do lado de destino (to)
    g_to = branch["g_to"]; b_to = branch["b_to"]

    # declaração das expressões não-lineares de fluxo de potência ativa e reativa
    # para cada sentido do ramo (De -> Para e Para -> De) através das equações AC Polar, utilizando as variáveis de módulo e ângulo de tensão (vm, va)
    # Equações AC Polar:
    
    # De -> Para
    p[(l, f, t)] = @NLexpression(model, (g+g_fr)/tm^2 * vm[f]^2 + (-g*tr+b*ti)/tm^2 * (vm[f]*vm[t]*cos(va[f]-va[t])) + (-b*tr-g*ti)/tm^2 * (vm[f]*vm[t]*sin(va[f]-va[t])))
    q[(l, f, t)] = @NLexpression(model, -(b+b_fr)/tm^2 * vm[f]^2 - (-b*tr-g*ti)/tm^2 * (vm[f]*vm[t]*cos(va[f]-va[t])) + (-g*tr+b*ti)/tm^2 * (vm[f]*vm[t]*sin(va[f]-va[t])))
    
    # Para -> De
    p[(l, t, f)] = @NLexpression(model, (g+g_to) * vm[t]^2 + (-g*tr-b*ti)/tm^2 * (vm[t]*vm[f]*cos(va[t]-va[f])) + (-b*tr+g*ti)/tm^2 * (vm[t]*vm[f]*sin(va[t]-va[f])))
    q[(l, t, f)] = @NLexpression(model, -(b+b_to) * vm[t]^2 - (-b*tr+g*ti)/tm^2 * (vm[t]*vm[f]*cos(va[t]-va[f])) + (-g*tr-b*ti)/tm^2 * (vm[t]*vm[f]*sin(va[t]-va[f])))
end

# =========================================================================
# 6. LEIS DE KIRCHHOFF DOS NÓS 
# =========================================================================
println("4. Montando balanço nodal (Leis de Kirchhoff)...")

# laço para cada barra do sistema, calculando o balanço nodal de potência
# ativa e reativa através das equações AC Polar, utilizando as variáveis 
# de fluxo de potência (p, q) e as variáveis de desvio (folga) de carga 
# reativa (sl_d) para permitir que a carga seja ajustada para atender
# às restrições de fluxo de potência.
for (i, bus) in ref[:bus]
    bus_arcs = ref[:bus_arcs][i] # lista de ramos conectados à barra i (origem ou destino)
    bus_arcs_dc = ref[:bus_arcs_dc][i]  # lista de linhas DC conectadas à barra i (origem ou destino)
    
    bus_gens = ref[:bus_gens][i] # lista de geradores conectados à barra i
    bus_loads = ref[:bus_loads][i] # lista de cargas conectadas à barra i

    # soma das condutâncias e susceptâncias dos shunts conectados à barra i
    gs = sum(shunt["gs"] for (k,shunt) in ref[:shunt] if shunt["shunt_bus"] == i; init=0.0)
    bs = sum(shunt["bs"] for (k,shunt) in ref[:shunt] if shunt["shunt_bus"] == i; init=0.0)

    # soma das cargas ativas e reativas conectadas à barra i
    pd_nominal = sum(load["pd"] for (k,load) in ref[:load] if load["load_bus"] == i; init=0.0)
    qd_nominal = sum(load["qd"] for (k,load) in ref[:load] if load["load_bus"] == i; init=0.0)

    # soma das potências ativas e reativas geradas pelos geradores conectados à barra i
    p_gen_total = isempty(bus_gens) ? 0.0 : sum(pg[g] for g in bus_gens)
    q_gen_total = isempty(bus_gens) ? 0.0 : sum(qg[g] for g in bus_gens)
    
    # soma das potências ativas e reativas injetadas pelas linhas DC conectadas à barra i
    p_dcline_total = isempty(bus_arcs_dc) ? 0.0 : sum(p_dc[a] for a in bus_arcs_dc)
    q_dcline_total = isempty(bus_arcs_dc) ? 0.0 : sum(q_dc[a] for a in bus_arcs_dc)

    # soma das folgas de carga reativa (sl_d) para cada barra com carga conectada à barra i
    slack_vlim  = isempty(bus_loads) ? 0.0 : sum(sl_d[l] for l in bus_loads)

    # restrições de balanço nodal (Leis de Kirchhoff) para cada barra do sistema
    # para valores de potência ativa.
    @NLconstraint(model, 
        sum(p[a] for a in bus_arcs) + p_dcline_total == p_gen_total - pd_nominal - gs*vm[i]^2 
    )

    # restrições de balanço nodal (Leis de Kirchhoff) para cada barra do sistema
    # para valores de potência reativa.
    @NLconstraint(model, 
        sum(q[a] for a in bus_arcs) + q_dcline_total == q_gen_total - (qd_nominal + slack_vlim) + bs*vm[i]^2 
    )
end

# =========================================================================
# 7. FUNÇÃO OBJETIVO DE SOFT-CONSTRAINTS
# =========================================================================
@objective(model, Min, 
    PENALIDADE * sum(sl_v[i]^2 for i in keys(sl_v)) + 
    PENALIDADE * sum(sl_d[l]^2 for l in keys(sl_d))
)

# =========================================================================
# 8. RESOLUÇÃO 
# =========================================================================
println("5. Resolvendo o Fluxo de Potência Controlado...\n")
tempo_total_execucao = @elapsed optimize!(model)

println("\n--- ESTATÍSTICAS DE RESOLUÇÃO ---")
println("Status da Convergência: ", termination_status(model))
println("Tempo interno do Solver (Ipopt): ", round(solve_time(model), digits=4), " segundos")
println("Tempo total da execução da função: ", round(tempo_total_execucao, digits=4), " segundos")
println("Erro de Controle (Slacks Ponderadas): ", objective_value(model))

# =========================================================================
# 9. RESUMO OPERACIONAL E FÍSICO
# =========================================================================
vetor_tensoes = [value(vm[i]) for i in keys(ref[:bus])]
tensao_min = minimum(vetor_tensoes)
tensao_max = maximum(vetor_tensoes)

geracao_p_total = sum(value(pg[g]) for g in keys(ref[:gen]); init=0.0)
geracao_q_total = sum(value(qg[g]) for g in keys(ref[:gen]); init=0.0)

perda_p_total = sum(
    value(p[(l, b["f_bus"], b["t_bus"])]) + value(p[(l, b["t_bus"], b["f_bus"])]) 
    for (l, b) in ref[:branch]; init=0.0
)

println("\n--- RESUMO OPERACIONAL GLOBAL ---")
println("Tensão Mínima (pu):         ", round(tensao_min, digits=4))
println("Tensão Máxima (pu):         ", round(tensao_max, digits=4))
println("Geração Ativa Total (pu):   ", round(geracao_p_total, digits=4))
println("Geração Reativa Total (pu): ", round(geracao_q_total, digits=4))
println("Perdas Ativas (Total pu):   ", round(perda_p_total, digits=4))

println("\n--- RESUMO EM UNIDADES REAIS (Base = $base_mva MVA) ---")
println("Geração Ativa Total (MW):   ", round(geracao_p_total * base_mva, digits=2))
println("Geração Reativa Total (MVAr):", round(geracao_q_total * base_mva, digits=2))
println("Perdas Ativas Totais (MW):  ", round(perda_p_total * base_mva, digits=2))
