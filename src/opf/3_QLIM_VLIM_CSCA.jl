...
PWF.silence() # Desativa mensagens de log do PWF

# Lemos o arquivo forçando a extração dos dados de controle (CSCA)
data = PWF.parse_file(arquivo, add_control_data=true)
...
# Mantém todas as barras de referência definidas no PWF. Somente a primeira
# será usada para definir a referência angular; as demais permanecem como
# barras de referência para que seus geradores possam participar do balanço.
ref_bus_ids = [b_id for (b_id, bus) in data["bus"] if bus["bus_type"] == 3]
isempty(ref_bus_ids) && error("O arquivo PWF não contém nenhuma barra de referência.")
barra_referencia_angular = first(ref_bus_ids)

if length(ref_bus_ids) > 1
    println("-> Múltiplas barras de referência detectadas. Mantendo todas; o ângulo da barra $barra_referencia_angular será fixado em 0,0.")
end

# Padroniza as funções de custo dos geradores para polinômios de segunda ordem.
# Embora o foco seja minimizar slacks de controle, o PowerModels exige essa padronização 
# interna para evitar erros durante a montagem do modelo.
PowerModels.standardize_cost_terms!(data, order=2)

# ---> PATCH DE LIMPEZA <---
# Remove chaves não-numéricas que o PWF adiciona e que quebram o PowerModels
# (Nenhuma dessas chaves não-numéricas são importantes para a análise, exemplos
# são "parameters" e "actions")

# laço para percorrer todos os componentes do dicionário de dados
# Utilizamos o '_' para a chave, pois apenas o 'comp_dict' (valor) nos interessa
for (_, comp_dict) in data
    if comp_dict isa Dict
        chaves_para_remover = String[]

        # laço para percorrer todas as chaves e valores do dicionário do componente
        for (k, v) in comp_dict # Chave k e valor v

            # Converte a chave de string para um número Inteiro.
            # Se a chave for "10", ela retorna 10. Se a chave for "parameters", 
            # ela retorna nothing.
        ...
        # laço para remover as chaves não-numéricas do dicionário do componente
...
# O método build_ref do PowerModels cria uma estrutura de referência que facilita o acesso 
# aos dados da rede, incluindo listas de barras, ramos, geradores, cargas e shunts. 
# Ele também constrói mapeamentos topológicos que permitem identificar rapidamente quais 
# elementos estão conectados a quais barras, o que é essencial para montar as equações 
# de fluxo de potência e as restrições de controle.
# Esse método processa o dicionário data e realiza um mapeamento topológico reverso de alta performance,
# permitindo que, dado um ID de barra, seja possível acessar rapidamente todos os elementos conectados a ela.
# Além disso , ele organiza os dados de forma que seja fácil iterar sobre geradores, cargas, shunts e ramos 
# associados a cada barra, o que é crucial para a formulação das equações de balanço nodal (Leis de Kirchhoff)
# e para a implementação das restrições de controle (QLIM, VLIM e CSCA).
...
# declaração das variáveis de módulo e ângulo de tensão para cada barra
# com seus respectivos limites de tensão (vmin, vmax) e valores iniciais (vm, va)
...
@variable(model, 0.9 <= vm[i in keys(ref[:bus])] <= 1.1, start=1.0)
@variable(model, va[i in keys(ref[:bus])], start=0.0)

# declaração das variáveis de potência ativa e reativa para cada gerador
# com seus respectivos limites de geração (pmin, pmax, qmin, qmax) e valores iniciais (pg, qg)
 # Potência Geração Ativa (Pg) declarada respeitando os limites do arquivo PWF.
@variable(model, ref[:gen][i]["pmin"] <= pg[i in keys(ref[:gen])] <= ref[:gen][i]["pmax"], start=ref[:gen][i]["pg"])

# Potência Geração Reativa (Pg) declarada respeitando os limites do arquivo PWF.
@variable(model, ref[:gen][i]["qmin"] <= qg[i in keys(ref[:gen])] <= ref[:gen][i]["qmax"], start=ref[:gen][i]["qg"])


# Variáveis dos Elos DC (note-1)
p_dc = Dict(); q_dc = Dict() # Cria os dicionários de potencias ativas e reativas
for (l, dcline) in ref[:dcline] # "l" é o ID da linha e "dcline" é o dicionário contendo os parâmetros físicos dela.
    ...
    p_dc[(l, f, t)] = @variable(model, start=dcline["pf"]) # variável de injeção de potência ativa para o lado from do elo.
    p_dc[(l, t, f)] = @variable(model, start=dcline["pt"]) # // variável de injeção de potência ativa para o lado to do elo
    q_dc[(l, f, t)] = @variable(model, start=dcline["qf"]) # // variável de injeção de potência reativa para o lado from do elo
    q_dc[(l, t, f)] = @variable(model, start=dcline["qt"]) # // variável de injeção de potência reativa para o lado to do elo
...
# Cria um vetor de variáveis de decisão contínuas, indexado pelos IDs de 
# todos os elementos shunt da rede. 
# `bs_var` representa a susceptância variável de cada shunt, que pode ser
# ajustada pelo solver dentro dos limites definidos no arquivo PWF.
@variable(model, bs_var[i in keys(ref[:shunt])])

# Slack de variação do shunt do seu valor padrão.
# `sl_bsh` é um vetor de variáveis de decisão contínuas, indexado pelos
# IDs de todos os elementos shunt da rede. Ele representa o desvio (ou slack)
# da susceptância do shunt em relação ao seu valor nominal definido no arquivo
# PWF. O solver pode ajustar essas variáveis para minimizar violações de tensão
# ou otimizar o fluxo de potência, penalizando desvios excessivos através da
# função objetivo.
@variable(model, sl_bsh[i in keys(ref[:shunt])], start=0.0)

# laço para percorrer todos os shunts e aplicar as restrições de controle
# de acordo com os dados do arquivo PWF
for (i, shunt) in ref[:shunt]
    bs_nom = shunt["bs"]

    # primeiro são assumidos os valores do shunt fixos e iguais ao valor
    # inicial caso não exista a informação no arquivo PWF sobre min e max.
    bmin, bmax = bs_nom, bs_nom
    
    # Se os dados de controle do ANAREDE existirem, os limites são atualizados.
    ...
    # Calcula os limites reais do shunt considerando o valor nominal 
    # e os limites de controle.
    real_bmin = min(bmin, bmax, shunt["bs"])
    real_bmax = max(bmin, bmax, shunt["bs"])
    
    # seta o lower bound de bs_var para cada i
    set_lower_bound(bs_var[i], real_bmin)
    # seta o upper bound de bs_var para cada i
    set_upper_bound(bs_var[i], real_bmax)
    # warm start no valor inicial presente no PWF
    set_start_value(bs_var[i], shunt["bs"])

    # Conecta a variável física ao slack
    @constraint(model, bs_var[i] == shunt["bs"] + sl_bsh[i])  
    
    # Se não houver margem de controle, fixa a variável bs_var no
    # valor nominal do shunt, garantindo que o solver não tente ajustá-la.
    # O parâmetro force=true assegura que a fixação seja aplicada mesmo se
    # houver limites previamente definidos para a variável.
...
# Geradores das barras de referência podem variar dentro dos limites do PWF.
# Os demais geradores permanecem fixados nos valores iniciais do caso.
# for (i, gen) in ref[:gen] # Itera sobre todos os geradores físicos da rede.
#     if !(gen["gen_bus"] in ref_bus_ids)
#         # se não for slack fixa sua potência no valor presente no PWF
#         fix(pg[i], gen["pg"]; force=true)
#     end
# end

# As barras de referência (Slack) não têm limites de geração, enquanto as 
# demais são fixadas nos valores iniciais do PWF
for (i, gen) in ref[:gen]
    # retira os limites de geração para as barras de referência (Slack)
...
# Fixa somente uma referência angular para eliminar a invariância global.
fix(va[tryparse(Int, barra_referencia_angular)], 0.0; force=true)

# =========================================================================
# 4. VARIÁVEIS E RESTRIÇÕES DE CONTROLE (QLIM, VLIM e CSCA)
# =========================================================================
PENALIDADE = 1e1
PENALIDADE_MENOR = 1e0 # Para garantir que o solver prefira mover o shunt a violar tensão
...
# Combina as listas de barras com geradores e shunts, removendo duplicatas.
...
# declaração das variáveis de desvio de carga reativa (folga) para cada 
# barra com carga. Essa variável permite que a carga seja ajustada para 
# atender às restrições de fluxo de potência e será utilizada mais abaixo 
# na restrição de balanço nodal (Leis de Kirchhoff) sendo somada à carga 
# reativa (qd) da barra, permitindo que a carga seja ajustada para atender 
# às restrições de fluxo de potência.
...
# variáveis de fluxo de potência ativa e reativa para cada ramo do sistema
p = Dict(); q = Dict()

# laço para cada ramo do sistema, calculando o fluxo de potência ativa e 
# reativa no sentido de cada barra (De -> Para e Para -> De) através das 
#equações AC Polar.
for (l, branch) in ref[:branch]
    f = branch["f_bus"]; t = branch["t_bus"] # barras de origem e destino do ramo
    g, b = PowerModels.calc_branch_y(branch) # condutância e susceptância do ramo
    tr, ti = PowerModels.calc_branch_t(branch) # transformador (tap) do ramo
    tm = branch["tap"] # fator de transformação do ramo

    # condutância e susceptância do shunt do lado de origem (from)
    g_fr = branch["g_fr"]; b_fr = branch["b_fr"]

    # condutância e susceptância do shunt do lado de destino (to)
    g_to = branch["g_to"]; b_to = branch["b_to"]

    # declaração das expressões não-lineares de fluxo de potência ativa e reativa
    # para cada sentido do ramo (De -> Para e Para -> De) através das equações AC Polar, utilizando as variáveis de módulo e ângulo de tensão (vm, va)
    # Equações AC Polar:
...
# laço para cada barra do sistema, calculando o balanço nodal de potência
# ativa e reativa através das equações AC Polar, utilizando as variáveis 
# de fluxo de potência (p, q) e as variáveis de desvio (folga) de carga 
# reativa (sl_d) para permitir que a carga seja ajustada para atender
# às restrições de fluxo de potência.
for (i, bus) in ref[:bus]
     bus_arcs = ref[:bus_arcs][i] # lista de ramos conectados à barra i (origem ou destino)
    bus_arcs_dc = ref[:bus_arcs_dc][i]  # lista de linhas DC conectadas à barra i (origem ou destino)
    
    bus_gens = ref[:bus_gens][i] # lista de geradores conectados à barra i
    bus_loads = ref[:bus_loads][i] # lista de cargas conectadas à barra i

    # Identifica shunts conectados à barra i
    bus_shunts = [k for (k,shunt) in ref[:shunt] if shunt["shunt_bus"] == i]

    # soma das cargas ativas e reativas conectadas à barra i
    ...
    # soma das potências geradas ativas e reativas conectadas à barra i
    p_gen_total = isempty(bus_gens) ? 0.0 : sum(pg[g] for g in bus_gens)
    q_gen_total = isempty(bus_gens) ? 0.0 : sum(qg[g] for g in bus_gens)
    
    # soma das potências ativas e reativas injetadas pelas linhas DC conectadas à barra i
    ...
    # Variável de desvio de carga reativa (folga) para cada barra com carga
    slack_vlim  = isempty(bus_loads) ? 0.0 : sum(sl_d[l] for l in bus_loads)

    # Soma das condutâncias dos shunts conectados à barra i, a serem multiplicadas 
    # pelo quadrado do módulo de tensão da barra (vm[i]^2). Se não houver 
    # shunts conectados, a soma é zero.
